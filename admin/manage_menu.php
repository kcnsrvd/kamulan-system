...
function uploadImage($fileInputName) {
    ...
    $targetDir = __DIR__ . '/../assets/images/foods/';
...
if (isset($_POST['update'])) {
    ...
    // Get old image
    $stmtOld = $pdo->prepare("SELECT image FROM menu_items WHERE id = ?");
    $stmtOld->execute([$_POST['id']]);
    $old = $stmtOld->fetch();

    // Upload new if chosen
    $newImage = uploadImage('image');
    $finalImage = $newImage ? $newImage : $old['image'];

    $stmt = $pdo->prepare("UPDATE menu_items 
        SET name = ?, description = ?, price = ?, category = ?, available = ?, image = ?
        WHERE id = ?");
...
if (isset($_POST['add'])) {
    ...
    if ($filename === null) {
        $filename = ""; // or default image
    }

    $stmt = $pdo->prepare("INSERT INTO menu_items 
        (name, description, price, category, available, image, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, NOW())");
...
<style>
  ...
  .container { width: 90%; margin: 30px auto; background: #fff; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
  h2 { text-align: center; color: #333; }
  .filters { display: flex; justify-content: space-between; flex-wrap: wrap; margin-bottom: 20px; align-items: center; }
  .category-buttons { display: flex; flex-wrap: wrap; gap: 8px; }
  .category-buttons a { background: #eee; color: #333; padding: 6px 12px; border-radius: 6px; text-decoration: none; }
  .category-buttons a.active, .category-buttons a:hover { background: #6b4f2c; color: #fff; }
  .search-group { display: flex; align-items: center; gap: 8px; }
  .search-form input { padding: 8px; border-radius: 6px; border: 1px solid #ccc; }
  .search-form button, .btn-add { padding: 8px 16px; background: #6b4f2c; color: white; border: none; border-radius: 6px; cursor: pointer; }
  table { width: 100%; border-collapse: collapse; }
  th, td { border-bottom: 1px solid #ddd; padding: 10px; text-align: center; vertical-align: middle; }
  th { background: #6b4f2c; color: white; }
  td img { width: 80px; height: 80px; object-fit: cover; border-radius: 8px; }
  .btn { padding: 6px 12px; border-radius: 6px; color: #fff; text-decoration: none; }
  .btn-edit { background: #007bff; }
  .btn-delete { background: #dc3545; }
  .toggle { cursor: pointer; padding: 5px 10px; border-radius: 6px; color: white; font-weight: bold; }
  ...
  .modal-content input, .modal-content textarea, .modal-content select { width: 100%; margin: 6px 0; padding: 8px; border-radius: 6px; border: 1px solid #ccc; }
  .back-btn { display: inline-flex; align-items: center; gap: 6px; background: none; border: none; color: #6b4f2c; font-weight: 600; cursor: pointer; margin-bottom: 10px; font-size: 15px; text-decoration: none; transition: color 0.2s ease; }
  .back-btn:hover { color: #4a3b20; }
...
<div class="container">
  ...
  <table>
    ...
    <tr>
      <td>
        <?php if (!empty($item['image']) && file_exists(__DIR__ . '/../assets/images/foods/' . $item['image'])): ?>
          <img src="../assets/images/foods/<?= htmlspecialchars($item['image']) ?>" alt="<?= htmlspecialchars($item['name']) ?>">
        <?php else: ?>
          <img src="../assets/images/foods/no-image.jpg" alt="No image">
        <?php endif; ?>
      </td>
...
function toggleAvailable(id, el) {
  ...
  .then(res => {
    if (res.trim() === 'success') {
      ...
      el.textContent = el.classList.contains('yes') ? 'Yes' : 'No';
    }
  });
}
</script>
