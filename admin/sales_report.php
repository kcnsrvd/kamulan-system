...
$from = $_GET['from'] ?? '';
$to = $_GET['to'] ?? '';
// --- Total net sales per branch (excluding delivery fee which goes to riders) ---
$branchSalesQuery = " 
  SELECT b.name AS branch, SUM(o.total - o.delivery_fee) AS total_sales
  FROM orders o
...
$branchParams = [];
if ($from && $to) {
    $branchSalesQuery .= " AND DATE(o.created_at) BETWEEN ? AND ?";
    $branchParams = [$from, $to];
} elseif ($from) {
    $branchSalesQuery .= " AND DATE(o.created_at) >= ?";
    $branchParams = [$from];
} elseif ($to) {
    $branchSalesQuery .= " AND DATE(o.created_at) <= ?";
    $branchParams = [$to];
...
$branchSalesQuery .= " GROUP BY b.name ORDER BY total_sales DESC";
$stmt2 = $pdo->prepare($branchSalesQuery);
$stmt2->execute($branchParams);
$branchSales = $stmt2->fetchAll(PDO::FETCH_ASSOC);
...
$query = " 
  SELECT 
    ...
    o.total,
    o.delivery_fee,
    (o.total - o.delivery_fee) AS net_total,
    o.payment_method, 
    o.status, 
    o.created_at
  FROM orders o
    ...
    <?php if (count($orders) > 0): ?>
      <?php foreach ($orders as $o): ?>
        <tr>
          ...
          <td><?= number_format(($o['net_total'] ?? ($o['total'] - ($o['delivery_fee'] ?? 0))), 2) ?></td>
  ...
  <?php if (count($branchSales) > 0): ?>
    <?php foreach ($branchSales as $b): ?>
      <tr>
        ...
        <td><?= number_format($b['total_sales'], 2) ?></td>
      </tr>
    <?php endforeach; ?>
  <?php else: ?>
    <tr><td colspan="2">No sales data available for selected range.</td></tr>
  <?php endif; ?>
</table>
