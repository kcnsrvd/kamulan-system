...
$total = $order['total']; // final amount customer paid (includes delivery fee)
// Prefer explicit delivery_fee column if present, otherwise fall back to difference
$delivery_fee = isset($order['delivery_fee']) ? (float)$order['delivery_fee'] : max($total - $subtotal, 0);
...
<tr>
    <td colspan="2" class="total">Delivery Fee (Rider Earnings)</td>
