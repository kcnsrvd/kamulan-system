...
// Always load the freshest user data from DB
...
if (!$user) {
    ...
    if ($action === 'update_profile') {
        ...
        if ($first_name === '' || $last_name === '' || $email === '' || $phone === '') {
            ...
            // Refresh in-memory user + session
            $stmt->execute([$userId]);
            $user = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($user) {
                $userSession = $user;
        ...
        if ($new === '' || $confirm === '' || $current === '') {
            ...
            if ($stored && password_verify($current, $stored)) {
                $valid = true;
            } elseif ($current === $stored) { // fallback for legacy plain-text
                $valid = true;
            }
  ...
  <body>
    ...
    <main class="profile-page">
      <div class="profile-header">
        ...
        <p style="font-size:0.9rem;color:#555;">Manage your account information and security.</p>
      </div>
        ...
        <section class="profile-card">
          ...
          <form method="post">
            ...
            <p class="profile-note">
              ...
              <a href="/kamulan-system/buyer/edit_address.php">Edit address &amp; contact details</a>.
            </p>
